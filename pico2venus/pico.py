...
def open_tcp(pico_ip):
  try:
    ...
    logging.error( "Connection to " + str(pico_ip) + ":5001 failed. Retrying in 1 sec.")
    time.sleep(5)
    # try again
...
logging.debug( "Start UDP listener")
...
pico_ip = addr[0]
logging.debug("See Pico at " + str(pico_ip))
...
while True:
    ...
    while True:
      message, addr = client.recvfrom(2048)
      logging.debug("Received packet with length " + str(len(message)))
...
#    logging.debug(json.dumps(sensorListTmp, indent=1))
    logging.debug("dumping pico sensorList to json file")

    try:                                                                            
        # Store the JSON data in a file                                             
        with open("/run/udev/data/pico-data.json", "w") as file:                         
            file.write(json.dumps(sensorListTmp))                                            
            file.close()                                   
    except:                                                
        logging.error('Failed to write json file')          
        sys.exit(1)

    empty_socket(client)

    # Wait until picked-up
    while os.path.exists('/run/udev/data/pico-data.json'):
        time.sleep(2)
